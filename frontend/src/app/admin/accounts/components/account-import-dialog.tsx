...
import { createAccounts, type Account } from "@/lib/account-pool-api";
...
import { getSessionAccessToken, getCpaAccessToken } from "@/lib/utils";
  ...
  const handleCpaSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsedFileCount === 0) {
        toast.error("这些 CPA JSON 文件里没有读取到可用 accessToken");
        return;
      }
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-5">
            <div className="space-y-2">
              ...
              <div className="text-sm leading-6 text-stone-500">
                每个文件应为一个 JSON 对象。系统会从对象中自动提取 `accessToken` 或 `accessToken`，
              </div>
            </div>
            <Button
              type="button"
    ...
    return (
      <div className="space-y-3">
        ...
        <MethodCard
          ...
          description="支持一次多选多个本地 JSON 文件，逐个读取对象里的 accessToken 后导入。"
          icon={Files}
