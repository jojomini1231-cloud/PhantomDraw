...
function downloadTokens(accounts: Account[]) {
  const content = `${accounts.map((account) => account.accessToken).join("\n")}\n`;
...
export default function AccountsPage() {
  ...
  const selectedTokens = useMemo(() => {
    ...
    return accounts.filter((item) => selectedSet.has(item.id)).map((item) => item.accessToken);
  }, [accounts, selectedIds]);

  const abnormalTokens = useMemo(() => {
    return accounts.filter((item) => item.status === "异常").map((item) => item.accessToken);
  }, [accounts]);
  ...
  const handleUpdateAccount = async () => {
    ...
    setIsUpdating(true);
    try {
      const data = await updateAccount(editingAccount.accessToken, {
        type: editType,
        status: editStatus,
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            variant="outline"
            className="h-10 rounded-xl border-stone-200 bg-white/80 px-4 text-stone-700 hover:bg-white"
            onClick={() => void handleRefreshAccounts(accounts.map((item) => item.accessToken))}
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={editStatus} onValueChange={(value) => setEditStatus((value as AccountStatus) || "正常")}>
            ...
            <div className="space-y-2">
              ...
              <Select value={editType} onValueChange={(value) => setEditType((value as AccountType) || "Free")}>
                <SelectTrigger className="h-11 rounded-xl border-stone-200 bg-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <section className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
          {metricCards.map((item) => {
            ...
            return (
              <Card key={item.key} className="rounded-2xl border border-stone-200 bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      <section className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-sm">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-semibold tracking-tight text-stone-800">账户列表</h2>
            <Badge variant="secondary" className="rounded-lg bg-stone-100 px-2.5 py-0.5 text-stone-600 font-medium">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative min-w-[280px]">
              ...
              <Input
                ...
                className="h-10 rounded-xl border-stone-200 bg-stone-50/50 pl-10 focus-visible:ring-1 focus-visible:ring-stone-300 focus-visible:bg-white transition-all"
              />
            </div>
            <Select
              value={typeFilter}
              onValueChange={(value) => {
                setTypeFilter((value as AccountType | "all") || "all");
                setPage(1);
              }}
            >
              <SelectTrigger className="h-10 w-full rounded-xl border-stone-200 bg-stone-50/50 sm:w-[140px] focus:ring-1 focus:ring-stone-300">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Select
              value={statusFilter}
              onValueChange={(value) => {
                setStatusFilter((value as AccountStatus | "all") || "all");
                setPage(1);
              }}
            >
              <SelectTrigger className="h-10 w-full rounded-xl border-stone-200 bg-stone-50/50 sm:w-[140px] focus:ring-1 focus:ring-stone-300">
        ...
        {isLoading && accounts.length === 0 ? (
          <Card className="rounded-2xl border border-stone-200 bg-white shadow-sm mt-4">
        ...
        <Card
          className={cn(
            "overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm mt-4",
            isLoading && accounts.length === 0 ? "hidden" : "",
          )}
        >
          <CardContent className="space-y-0 p-0">
            <div className="flex flex-col gap-4 border-b border-stone-100 px-6 py-4 lg:flex-row lg:items-center lg:justify-between bg-stone-50/30">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[920px] text-left">
                <thead className="border-b border-stone-200 bg-stone-50/50 text-[11px] text-stone-500 uppercase tracking-[0.1em]">
                  <tr>
                    ...
                    <th className="w-56 px-6 py-4 font-medium">token</th>
                    <th className="w-28 px-6 py-4 font-medium">类型</th>
                    <th className="w-24 px-6 py-4 font-medium">状态</th>
                    <th className="w-56 px-6 py-4 font-medium">账号信息</th>
                    <th className="w-24 px-6 py-4 font-medium">额度</th>
                    <th className="w-40 px-6 py-4 font-medium">恢复时间</th>
                    <th className="w-18 px-6 py-4 font-medium">成功</th>
                    <th className="w-18 px-6 py-4 font-medium">失败</th>
                    <th className="w-24 px-6 py-4 font-medium">操作</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-100">
                  {currentRows.map((account) => {
                    ...
                    return (
                      <tr
                        key={account.id}
                        className="text-sm text-stone-600 transition-colors hover:bg-stone-50/80 group"
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className="font-medium tracking-tight text-stone-800">
                              {maskToken(account.accessToken)}
                            </span>
                            <button
                              type="button"
                              className="rounded-lg p-1.5 text-stone-400 opacity-0 transition-all group-hover:opacity-100 hover:bg-stone-100 hover:text-stone-700"
                              onClick={() => {
                        ...
                        <td className="px-6 py-4 text-xs leading-5 text-stone-500">
                          {(() => {
                            ...
                            return (
                              <div className="space-y-1">
                                ...
                                <div className="text-stone-400">{restore.absolute}</div>
                              </div>
                            );
                          })()}
                        </td>
                        <td className="px-6 py-4 text-stone-500 font-medium">{account.success}</td>
                        <td className="px-6 py-4 text-stone-500 font-medium">{account.fail}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1 text-stone-400 opacity-0 transition-opacity group-hover:opacity-100">
                            <button
                              type="button"
                              className="rounded-lg p-2 transition-colors hover:bg-stone-100 hover:text-stone-700"
                            ...
                            <button
                              type="button"
                              className="rounded-lg p-2 transition-colors hover:bg-stone-100 hover:text-stone-700"
                              onClick={() => void handleRefreshAccounts([account.accessToken])}
                              disabled={isRefreshing}
                              title="刷新"
                            >
                              <RefreshCw className={cn("size-4", isRefreshing ? "animate-spin text-stone-700" : "")} />
                            </button>
                            <button
                              type="button"
                              className="rounded-lg p-2 transition-colors hover:bg-rose-50 hover:text-rose-500"
                              onClick={() => void handleDeleteTokens([account.accessToken])}
                              disabled={isDeleting}
                              title="删除"
                            >
